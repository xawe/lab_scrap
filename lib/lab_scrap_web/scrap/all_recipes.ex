defmodule LabScrapWeb.Scrap.AllRecipes do
  @moduledoc """
  ...
  @store LabScrap.Agents.ResultStore

  @url_call_function &LabScrapWeb.Scrap.AllRecipes.build_recipe/1
  ...
  def get_smoothies_recipe(pool_count) do
    ...
    LabScrap.Mediator.process(urls, pool_count, @url_call_function)
    {:created, status}
  end

  def get_smoothies_recipe() do
    {status, urls} = read_main_url()
    Enum.each(urls, fn url -> LabScrap.Mediator.process(url, @url_call_function) end)
    {:created, status}
  end
