defmodule LabScrap.Mediator do
@moduledoc """
  ...
  def process(url, url_fun) do
    {_, pid} = DynamicSupervisor.start_child(DynamicCaller, LabScrap.ProcessServer)
    GenServer.cast(pid, {:process, url, url_fun})
  end

  def process(url, max_process, url_fun) do
    IO.puts("Numero de processos #{max_process}")
  ...
  def warmup_process(max_process) do
    Enum.map(1..max_process, fn _ ->
      DynamicSupervisor.start_child(DynamicCaller, LabScrap.ProcessServer)
    end)
